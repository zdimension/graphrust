...
use graph_format::nalgebra::{Vector, U13, U15};
...
fn find_fixed_point<State: Copy + Debug, Value>(
    ...
    for value in iter {
        ...
        if n > min && current_dist < precision {
            // reached desired precision after minimum iterations
    ...
    // if we got here, we didn't converge and reached the end of the iterator
    (n, None)
}
...
async fn main() {
    ...
    let get_path_lens_n4j = async || {
        ...
        #[coroutine]
        static move || {
            // since computing thousands of path lengths with Neo4j is slow, we cache them in a file
            // don't delete it!
...
RETURN a.uid AS from, b.uid AS to, 
    ...
    /* let testt = get_path_lens_n4j().await;
    let testt2 = iter::from_coroutine(pin!(testt)).take(10).collect_vec();
    dbg!(&testt2);*/
    ...
    python! {
        ...
        # plot degdist from n4j_deg_dist.csv
        # log-log scatter
        import pandas as pd
        df = pd.read_csv("n4j_deg_dist.csv")
        df = df[df["deg"] > 0]
        df_filter = df[df["deg"] <= 50]
        x = np.log(df_filter["deg"])
        y = np.log(df_filter["count(n)"])
        coeffs = np.polyfit(x, y, 1)
        # add the fit line to the plot
        x_fit = np.linspace(x.min(), np.log(df["deg"].max()), 100)
        y_fit = coeffs[0] * x_fit + coeffs[1]

        # plot scatter and trend line
        fig = px.scatter(df, x="deg", y="count(n)", log_x=True, log_y=True, title="Degree Distribution with Power-Law Fit", labels={"deg": "Degree", "count(n)": "Number of Nodes"})
        # line in dashed red
        fig.add_traces(px.line(x=np.exp(x_fit), y=np.exp(y_fit), labels={"x": "Degree", "y": "Number of Nodes"}).update_traces(line=dict(color="red", dash="dash"), name="Power-Law Fit").data)
        # convert equation to A*x^B form
        eq_str = "y = {:.3f} * x^{:.3f}".format(np.exp(coeffs[1]), coeffs[0])
        fig.add_annotation(x=0.05, y=0.95, xref="paper", yref="paper", text=eq_str, showarrow=False, font=dict(size=12, color="red"), align="left")
        fig.show()
