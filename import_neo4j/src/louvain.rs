...
impl Graph {
    pub fn new(persons: Vec<Vec<u32>>) -> Self {
        let mut nodes = Vec::new();
        let mut total_links = 0;
        for (i, pers) in persons.iter().enumerate() {
            let mut comm = Community::new(Some(vec![PersonId(i)]));
            comm.neighbors = pers.iter().map(|&x| (CommunityId(x as usize), 1)).collect();
            nodes.push(comm);
            total_links += pers.len();
        }
        ...
        for i in 0..communities.len() {
            ...
            comm.neighbors = comm
                ...
                .splice(
                    ...
                    links
                        .iter()
                        .map(|(&comm, &weight)| (CommunityId(comm), weight)),
                )
                .collect();
...
pub struct Community {
    ...
    neighbors: Vec<(CommunityId, usize)>,
    degree: usize,
    parent: Option<usize>,
    self_loops: usize,
}
