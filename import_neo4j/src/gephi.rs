// From https://github.com/graphext/louvain-rs
// under https://github.com/graphext/louvain-rs/blob/master/LICENSE_MIT
...
impl CommunityStructure {
    pub fn new<T>(graph: &Graph<T>, modularity: &mut Modularity) -> CommunityStructure {
        ...
        for _node in graph.node_indices() {
            ...
            cs.weights.push(0.0);
            cc.get_mut(&cs.nodeCommunities[index])
                .unwrap()
                .seed(index, &cs);
    ...
    fn addNodeTo(&mut self, node: usize, com_id: CommunityId, cc: &mut CommunityCatalog) {
        #[inline]
        fn add_node<V: AddAssign + Copy + From<u8>>(
            map: &mut BTreeMap<CommunityId, V>,
            key: CommunityId,
            weight: V,
        ) {
    ...
    pub fn removeNodeFromItsCommunity(&mut self, node: usize, cc: &mut CommunityCatalog) {
        #[inline]
        fn remove_node(
            weights_map: &mut BTreeMap<CommunityId, f32>,
...

        {
            ...
            for e in &self.topology[node] {
                ...
                remove_node(
                    &mut self.nodeConnectionsWeight[*neighbor],
                ...
                let adjCom = cc.get(&self.nodeCommunities[*neighbor]).unwrap();
                remove_node(
                    &mut adjCom.connectionsWeight.borrow_mut(),
                ...
                if adjCom.id != community.id {
                    remove_node(
                        &mut community.connectionsWeight.borrow_mut(),
                ...
                remove_node(
                    &mut self.nodeConnectionsWeight[node],
    ...
    fn zoomOut(&mut self, cc: &mut CommunityCatalog) {
        ...
        for i in 0..M {
            ...
            for e in &newTopology[i] {
                ...
                com.connectionsWeight
                    .borrow_mut()
                    .insert(nodeCommunities[e.target], e.weight);
                com.connectionsCount
                    .borrow_mut()
                    .insert(nodeCommunities[e.target], 1);
            }
        }
...
impl Modularity {
    ...
    pub fn execute<T>(&mut self, graph: &Graph<T>) -> (f64, f64) {
        ...
        if graph.node_count() > 0 {
            let (modularity, modularityResolution) =
                self.computeModularity(graph, &mut structure, &mut communities);
    ...
    fn computeModularity<T>(
        &mut self,
        graph: &Graph<T>,
        cs: &mut CommunityStructure,
        communities: &mut Vec<i32>,
    ) -> (f64, f64) {
        ...
        let computedModularity =
            self.finalQ(&mut comStructure, &degreeCount, graph, totalWeight, 1.);
        let computedModularityResolution = self.finalQ(
            &mut comStructure,
            &degreeCount,
            graph,
            totalWeight,
        ...
        for i in 0..communities.len() {
            communities[i] = if noiseMap[i] {
                -1
            } else {
                comStructure[i] as i32
            }
        }
    ...
    fn fillDegreeCount<T>(
        &self,
        graph: &Graph<T>,
        cs: &CommunityStructure,
        comStructure: &Vec<usize>,
        nodeDegrees: &Vec<f64>,
    ) -> Vec<f64> {
    ...
    fn q(
        ...
        let mut qValue: f64 =
            currentResolution * edgesTo - (nodeWeight * weightSum) / (2.0 * cs.graphWeightSum);
        ...
        if (cs.nodeCommunities[node] == *com_id) && (nodeCommunities_len > 1) {
            qValue = currentResolution * edgesTo
                - (nodeWeight * (weightSum - nodeWeight)) / (2.0 * cs.graphWeightSum);
        }
        if (cs.nodeCommunities[node] == *com_id) && (nodeCommunities_len == 1) {
            qValue = 0.0;
        }
    ...
    fn finalQ<T>(
        ...
        for node in graph.node_indices() {
            for edge in graph.edges(node) {
                // FIXME: check if getEdges() is inner edges
                if node == edge.target() {
                    continue;
                }
        ...
        for i in 0..degrees.len() {
            ...
            res += usedResolution * (internal[i] / totalWeight)
                - (degrees[i] / (2.0 * totalWeight)).powi(2);
        }
        res
    }
}
...
fn check_weights(cc: &CommunityCatalog, cs: &CommunityStructure) -> bool {
    for i in 0..cs.nodeCommunities.len() - 1 {
        ...
        if let Some(w0_1) = com0
            .connectionsWeight
            .borrow()
            .get(&cs.nodeCommunities[i + 1])
        {
            let nw0_1 = cs.nodeConnectionsWeight[i]
                .get(&cs.nodeCommunities[i + 1])
                .unwrap();
